...
export const createBookingCheckoutSession = protectedActionClient
  ...
  .action(async ({ parsedInput: { serviceId, date }, ctx: { user } }) => {
    ...
    const shouldApplyDiscount =
      dbUser?.welcomeDiscountClaimed && !dbUser?.welcomeDiscountUsed;

    const discountAmount = shouldApplyDiscount ? 500 : 0; // R$ 5,00
    const finalAmount = Math.max(0, service.priceInCents - discountAmount);
    ...
    const checkoutSession = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        appliedDiscount: shouldApplyDiscount ? "500" : "0",
      },
      line_items: [
        {
          price_data: {
