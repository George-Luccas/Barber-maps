...
const WinnerEffects = () => {
  // Simple particle explosion effect using Framer Motion
  const colors = ["#8b5cf6", "#fbbf24", "#ef4444", "#3b82f6"];
...
const BarbershopRanking = ({ barbershops, city }: BarbershopRankingProps) => {
  ...
  const getCrownColor = (index: number) => {
    switch (index) {
      case 0: return "text-yellow-400 fill-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,1)]";
  ...
  const RankItem = ({ barbershop, index, isTop3 = false }: { barbershop: BarbershopWithBookings, index: number, isTop3?: boolean }) => {
    ...
    <motion.div
      ...
      className={`w-full ${isWinner ? 'relative z-20 scale-105' : ''}`}
    >
      <Link
        href={`/barbershops/${barbershop.id}`}
        className={`group relative flex items-center gap-3 bg-muted/20 backdrop-blur-md p-3 sm:gap-4 sm:p-4 rounded-2xl border transition-all duration-300 dark:bg-black/40 
            ${isTop3 ? 'sm:flex-col sm:text-center sm:p-6 sm:justify-center' : ''}
            ${isWinner ? 'border-yellow-400/50 shadow-[0_0_30px_rgba(250,204,21,0.3)] bg-gradient-to-input from-yellow-400/10 to-transparent' : 'border-white/5 hover:border-neon-purple/50'}
        `}
      >
        {isWinner && <WinnerEffects />}
        ...
        <div className={`flex items-center justify-center ${isTop3 ? 'sm:mb-2' : 'w-8 sm:w-10'}`}>
          ...
          >
            <Crown className={`${getCrownColor(index)} ${isTop3 && index === 0 ? 'w-10 h-10 sm:w-14 sm:h-14' : 'w-6 h-6 sm:w-7 sm:h-7'}`} />
          </motion.div>
        </div>

        {/* Image */}
        <div className={`relative rounded-full overflow-hidden border-2 transition-colors shrink-0 
            ${isWinner ? 'border-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.5)]' : 'border-muted group-hover:border-neon-purple'}
        ...
        <div className={`${isTop3 ? 'sm:mt-2' : 'text-right shrink-0'}`}>
          <div className={`font-black text-sm sm:text-xl leading-none ${isWinner ? 'text-yellow-400' : 'text-neon-purple'}`}>{barbershop.bookingsCount}</div>
